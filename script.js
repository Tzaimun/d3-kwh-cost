...
class LineData {
  constructor(files) {
    this.country = files[0].GEO
    this.year =  parseInt(files[0].TIME)
    this.co2_value = parseFloat(files[0].Value.replace(/[,]/g, ''))
  }
}
class Country {
  constructor(files) {
    this.line_data = {}
    for (let i = 0; i < files[0][0].length; i++) {
      this.line_data.push(LineData(files))
    }
    if (files[1].TIME == '2019S1' && files[1].TAX == 'All taxes and levies included' && files[1].CURRENCY == 'EURO') {
      this.kWh_price = parseFLoat(files[1].Value)
    }
  }
}*/
    /*
      for(i = 0; i < data.length; i++) {
        data[i].co2_value = parseFloat(data[i].co2_value.replace(/[,]/g, ''))
      ...
      console.log(dataset)
      console.log(dataset[0].values[0])
      //Create scale functions
      xScale = d3.scaleTime()
            .domain([
              d3.min(dataset, d => d3.min(d.year)),
              d3.max(dataset, d => d3.max(d.year))
            ])

            .range([0, w]);

      yScale = d3.scaleLinear()
            .domain([0, d3.max(dataset, d => d.co2_value)])
            .range([h, 0]);

      //Define line generator
      line = d3.line()
            .x(d => xScale(d.co2_value ))
...
/*
d3.csv("/nrg_bal_c/energy_oil_tonnes_eq.csv", function(d) {
  return {
    country : d.GEO,
    year : d.TIME,
    co2_value : d.Value
